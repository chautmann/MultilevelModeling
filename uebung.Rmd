...
Berechnen Sie das Intercepts- and Slopes-as-Outcomes Modell. Berücksichtigen Sie hierzu die Variablen `gpa`, `sex` und `time` und geben in der Funktion `lme()` die festen und die zufälligen Effekte an. Betrachten Sie die Ergebnisse der Koeffizienten und hier insbesondere die festen Effekte. Versuchen Sie die Bedeutung die Prädiktoren `sex` und `time:sex` dem Gleichungssystem aus der Präsentation zuzuordnen.

```{r}
mod3 <- lme(_____, random = ~ time | _____, data = gpa_data_long)
